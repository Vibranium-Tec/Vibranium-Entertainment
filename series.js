...
function createVideoItem(item, isSearch = false) {
    ...
    thumbnail.href = `Wseries.html?id=${item.id}`;
    thumbnail.innerHTML = `
        <img loading="lazy" src="${getImage(item.poster_path)}" alt="${item.name}">
        <span class="playIcon">
...
async function displayTrending() {
    ...
    if (items && items.length > 0) {
        ...
        videoGrid.innerHTML = '<p style="text-align: center; grid-column: 1/-1;">No shows found</p>';
    }
}
...
searchInput.addEventListener('input', async () => {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        if (query) {
            try {
                const response = await fetch(`${TMDB_BASE_URL}/search/tv?api_key=${TMDB_API_KEY}&query=${query}`);
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                const data = await response.json();
                ...
                searchResults.innerHTML = '<p style="text-align: center;">Error searching shows</p>';
            }
        } else {
            searchResults.innerHTML = '';
